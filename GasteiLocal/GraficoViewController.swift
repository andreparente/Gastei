...
class GraficoViewController: UIViewController,ChartViewDelegate,UITextFieldDelegate {
    ...
    func setPieChart(_ dataPoints: [String], values: [Double]) {
        ...
        if(values.count == 0) {
            ...
            chartView.centerText = "TESTEZAO"
            var dataEntries: [ChartDataEntry] = []
            for i in 0..<values.count {
                let dataEntry = ChartDataEntry(x: Double(i), y: values[i].roundToPlaces(2))
                print("x:  ", values[i])
                print("y:  ", Double(i))
                dataEntries.append(dataEntry)
            }
            let chartDataSet = PieChartDataSet(values: dataEntries, label: "Categorias")
            ...
            print(chartData.dataSets.count, chartData.dataSets)
            for i in 0..<chartData.dataSets.count {
                print(chartData.dataSets[i].label!)
            }
